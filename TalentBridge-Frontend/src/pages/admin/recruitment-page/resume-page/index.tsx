...
import { findAllResumes, updateResumeStatus, getResumeStatsByStatus, type ResumeStatusStats } from "@/services/resumeApi.ts";
...
const ResumeManagerPage = () => {
  ...
  const fetchResumes = async (
    ...
    try {
      ...
      // Thêm sort=createdAt,desc để CV mới nhất hiển thị đầu tiên
      const res = (await findAllResumes({ page, size, filter, sort: "createdAt,desc" })).data.data;
  ...
  // Fetch thống kê khi component mount hoặc sau khi cập nhật status
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await getResumeStatsByStatus();
    ...
    fetchStats();
  }, [resumes]); // Re-fetch khi resumes thay đổi (sau khi update status)
